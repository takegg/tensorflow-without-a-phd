...
我们的神经网络接收手写数字并将其分类，即状态如果它识别出它们如a0,a1和a2直到a9。它基于内部变量（权重和偏见，稍后解释）需要具有正确的分类值才能正常工作。此“正确值”是通过训练过程学习到的，同样之后详细解释。
> 训练数据 => 更新权重和偏见 => 更好的识别 (循环)
...
“训练”神经网络实际意味着使用训练图片和标签来调整权重和偏见，以便最小化交叉熵损失函数。这里是它如何工作。
...
梯度的数学性质是它指向“上”，因为我们要到达交叉熵的低点，所以我们去反方向。我们用一个梯度函数更新权重和偏见，并且使用下一批训练图片做相同的事。希望这能让我们去交叉熵为最小的坑的底部。
...
> 提示：当你深入，随机值初始化权重将变的重要。如果你没这么做，优化器会卡在初始位置。**tf.truncated_normal**是一个TensorFlow函数，用于产生随默认（高斯）分配介于-2x标准差到+2x标准差的随机值。
...
在卷积神经网络的层中，一个“神经元”仅在图像的一个小区域上对其正上方的像素进行加权求和。然后它的默认行为通过添加偏见并通过其激活功能供养结果。最大的区别是每个神经元重复使用相同的权重，而在之前看到的完全连接的网络中，每个神经元都有自己的一组权重。
